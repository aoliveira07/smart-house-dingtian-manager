...
from dingtian_manager.app.core.mqtt_discovery import tone_topics, topics
...
async def test_feedback_only_duplicate_retained_baseline_and_persistence(tmp_path):
    ...
    assert "select.cabeado1_r1_tonalidade" in client.entities
...
async def test_interval_measured_after_off_feedback_and_retained_commands_ignored(tmp_path):
    client, port, manager, mid, c, address = await configured(tmp_path)
    control = tone_topics(manager.state, manager.state["modules"][mid], c)["command_topic"]
...
async def test_validation_reorder_identity_and_select_cleanup(tmp_path):
    client, port, manager, mid, c, address = await configured(tmp_path)
    m = deepcopy(manager.state["modules"][mid])
