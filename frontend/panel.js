...
export class DingtianPanel extends HTMLElement {
  constructor() {
    ...
    this.dirty = false; this.busy = false; this.error = ''; this.active = ''; this.loaded = false;
  ...
  connectedCallback() {
    window.addEventListener('popstate', this.onPop); window.addEventListener('beforeunload', this.onUnload);
  ...
  async load() {
    try {
      ...
      if (!this.dirty && !this.busy) this.resetDraft();
      if (!this.dirty && !this.busy) this.render(); else this.updateStates();
    } catch (e) {this.error = e.message || String(e); this.render();}
  }
  resetDraft() {
  ...
  updateStates() {
    ...
    for (const c of current.channels) {
      const node = this.shadowRoot.querySelector(`[data-state="${c.number}"]`);
      if (node) node.textContent = c.state === 'unknown' ? 'Aguardando estado' : c.state;
    }
  }
  readRoute() {
    const match = location.pathname.match(/\/smart-house-dingtian\/modules\/([a-f0-9]{32})/);
    const next = match?.[1] || '';
    if (next === this.active) return;
    if (this.dirty && !window.confirm('Descartar alterações não salvas deste módulo?')) {
      history.pushState(null, '', this.active ? `${ROOT}/modules/${this.active}` : ROOT);
  ...
  navigate(mid) {
    ...
    if (this.dirty && !window.confirm('Descartar alterações não salvas deste módulo?')) return;
    this.dirty = false; history.pushState(null, '', mid ? `${ROOT}/modules/${mid}` : ROOT);
  ...
  render() {
    ...
    main.append(el('header', {}, el('div', {}, el('p', {className: 'eyebrow'}, 'SMART HOUSE / AUTOMAÇÃO CABEADA'), el('h1', {}, 'Dingtian Manager'), el('p', {className: 'muted'}, 'Seus módulos, organizados canal por canal.')), button('☰ Menu', () => this.dispatchEvent(new CustomEvent('hass-toggle-menu', {bubbles: true, composed: true})))));
    ...
    if (this.data.prepared_removal) main.append(el('div',{className:'notice'},'Preparação para remoção concluída somente quando não houver pendências. Depois, remova a integração em Dispositivos e serviços.'));
  ...
  renderModule(main) {
    ...
    for(const c of m.channels){
      ...
      const type=el('select',{'aria-label':`Tipo R${c.number}`,onchange:e=>{c.entity_type=e.target.value;this.changed();}},el('option',{value:'light'},'Luz'),el('option',{value:'switch'},'Interruptor'));type.value=c.entity_type;
      ...
      if(c.entity_id && c.enabled) details.append(el('div',{className:'bar'},el('strong',{},'Comando físico real:'),...['ON','OFF'].map(payload=>button(payload==='ON'?'Ligar este canal':'Desligar este canal',()=>{if(this.dirty){this.error='Salve ou cancele o cadastro antes de operar.';this.render();return;}if(window.confirm(`Enviar ${payload} ao relé R${c.number} de ${m.technical_id}? Esta ação opera uma carga real.`))this.perform('operate',{module_uuid:m.module_uuid,number:c.number,payload},true);}))));
      channels.append(el('article',{className:'channel'},el('div',{className:'position'},`R${c.number}`),el('label',{className:'inline'},use,'Utilizado'),el('label',{},'Tipo',type),el('label',{className:'channel-name'},'Nome do canal',display),el('div',{className:'state muted','data-state':c.number,role:'status'},c.state==='unknown'?'Aguardando estado':c.state),details));
    }
